...
export const DetailsContainer: React.FC<DetailsContainerProps> = ({ title, onSubmit, buttonDisabled, children, backPath = '/' }) => (
    <Layout title={title} className="relative">            
        <form className="flex items-center w-full max-w-xl" onSubmit={onSubmit}>
            <div className="mr-12">
                <Link to={backPath} className="group focus:outline-none no-underline">
